...
class FlickrClient: UIViewController {
        ...
        let task = session.dataTask(with: request as URLRequest) { (data, response, error) in
            ...
            for photo in photoAOD {
                if let url = photo["url_m"] {
                    FlickrClient.saveImageURLToCore(url : url as! String)
                }
            }
    ...
    class func getImageDataFromURL(url : String, completion : @escaping (_ data : Data) -> Void){
    ...
    class func getDetailsFromFlickr(latitude : CLLocationDegrees, longitude : CLLocationDegrees, completion : @escaping (_ pages: Int?, _ numberOfImages: Int?)->Void){
